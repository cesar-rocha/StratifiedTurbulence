...
class Boussinesq2d(object):
    ...
    def __init__(
            ...
            twrite=3,             # interval for cfl and ke printout (in timesteps)
    ...
    def run(self):
        ...
        while(self.t < self.tmax): 
            ...
            if (self.ndt%self.twrite == 0.):
                ...
                self.tseries.append(self.t)
                self.keseries.append(self.ke)
    ...
    def _stepforward(self):
        ...
        self.qh = self.N1*(self.L1*self.qh + self.c1*self.dt*self.nl1h).copy()
        self.qh = self.filt*self.qh
        self.bh = self.N1*(self.L1*self.bh + self.c1*self.dt*self.nl1h_b).copy()
        ...
        self.nl1h_b = -self.jacobian_b() - self.kj*self.ph
        self.qh = self.N2*(self.L2*self.qh + self.c2*self.dt*self.nl1h +\
                self.d1*self.dt*self.nl2h).copy()
        self.qh = self.filt*self.qh
        self.bh = self.N2*(self.L2*self.bh + self.c2*self.dt*self.nl1h_b +\
        ...
        self.nl2h_b = self.nl1h_b.copy()
        self.nl1h_b = -self.jacobian_b() - self.kj*self.ph
        self.qh = self.N3*(self.L3*self.qh + self.c3*self.dt*self.nl1h +\
                self.d2*self.dt*self.nl2h).copy()
        self.qh = self.filt*self.qh
        self.bh = self.N3*(self.L3*self.bh + self.c3*self.dt*self.nl1h_b +\
    ...
    def _init_rk3w(self):
        ...
        self.L1 = 1. + self.a1*self.Lin
        self.L2 = 1. + self.a2*self.Lin
        self.L3 = 1. + self.a2*self.Lin

        self.N1 = (1. - self.b1*self.Lin)**-1
        self.N2 = (1. - self.b2*self.Lin)**-1
        self.N3 = (1. - self.b3*self.Lin)**-1
    ...
    def _initialize_core_diagnostics(self):
        ...
        self.add_diagnostic('ubar',
            description='x-averaged zonal velocity',
            function= (lambda self: self.u.mean(axis=1))
        )

        self.add_diagnostic('wbar',
            description='x-averaged vertical velocity',
            function= (lambda self: self.v.mean(axis=1))
        )

        self.add_diagnostic('bbar',
            description='x-averaged vertical velocity',
            function= (lambda self: self.b.mean(axis=1))
        )

        self.add_diagnostic('uwbar',
            description='x-averaged Reynolds stress',
            function= (lambda self: (self.vp*self.up).mean(axis=1))
        )
        
        self.add_diagnostic('wbbar',
            description='x-averaged buoyancy flux',
            function= (lambda self: (self.vp*self.bp).mean(axis=1))
        )

    def _calc_diagnostics(self):
        # here is where we calculate diagnostics
        if (self.t>=self.dt) and (self.t>=self.tavestart) and (self.ndt%self.taveints==0):
            self._increment_diagnostics()


    def _calc_derived_fields(self):
        """Should be implemented by subclass."""
        self.vbar = self.v.mean(axis=1)
        self.ubar = self.u.mean(axis=1)
        self.bbar = self.b.mean(axis=1)
        self.up = self.u - self.ubar[...,np.newaxis]
        self.vp = self.v - self.vbar[...,np.newaxis]
        self.bp = self.b - self.bbar[...,np.newaxis]

    def _initialize_model_diagnostics(self):
        """Should be implemented by subclass."""
        pass

    def _set_active_diagnostics(self):
